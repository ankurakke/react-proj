...
function App() {
  ...
  const [data,setData] = useState(mockData(9));

  const updatePrice = (row,value) => {
    const tempData = [...data];
    tempData.filter(e=>e.id==row.id)[0].price = value;
    setData(tempData);
  }
    ...
    () => [
      {
        ...
        columns: [
          ...
          {
            Header: 'price',
            accessor: row => <input value={row.price} onChange={(e)=>updatePrice(row,e.target.value)}></input>,
            sorted:true,
            editable:true
